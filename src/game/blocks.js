...
// Create a standard LEGO block with studs
export const createLegoBlockGeometry = () => {
  // Create a group to hold the block and studs
  ...
  studGeometry.translate(0, BLOCK_HEIGHT + STUD_HEIGHT/2, 0);
  
  // Add studs to the block
  const studPositions = [
    [-0.25, 0, -0.25],
    [0.25, 0, -0.25],
    [-0.25, 0, 0.25],
  ...
  // Add the block geometry
  mergedGeometry.merge(blockGeometry);
  
  // Add the stud geometries
  studPositions.forEach(position => {
    const tempStud = studGeometry.clone();
    tempStud.translate(position[0], position[1], position[2]);
    mergedGeometry.merge(tempStud);
  });
...
// Enhanced LEGO Block component with detailed geometry
export const LegoBlock = ({ position, type, onClick }) => {
  ...
  return (
    <mesh 
      ...
      receiveShadow
    >
      <bufferGeometry attach="geometry" {...createLegoBlockGeometry()} />
